 ...
 */
//a. select 文件描述符个数有关(不能改变) 1024 用位图
/*
...
/*int poll(struct pollfd *fds, nfds_t nfds, int timeout);
	...
	timeout 		毫秒级等待
        ...
        */
//c. epoll            可以改变文件描述符数量 内部文件描述符是红黑树结构，epoll_create创建，返回根节点
/**
 * 基础API
 * int epoll_create(int size)		size：监听数目，返回epfd
 * int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event) 2.控制某个epoll监控的文件描述符上的事件：注册、修改、删除。op:EPOLL_CTL_ADD 注册新的fd到epfd EPOLL_CTL_MOD (修改已经注册的fd的监听事件),   EPOLL_CTL_DEL (从epfd删除一个fd)
 * int epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout)  3.等待所监控文件描述符上有事件的产生，类似于select()调用。第二个是传出参数，指向满足条件的数组
 * 
 * 	event：	告诉内核需要监听的事件
 *		struct epoll_event {
...
//epoll 非阻塞io   event.events = EPOLLIN | EPOLLET;   ET 边沿触发 ，默认是水平触发
